...
namespace sot
{
    ...
    public:
         ...
         */
        RBFInterpolant(int maxPoints, int dim) : RBFInterpolant(maxPoints, dim, arma::zeros(dim), arma::ones(dim)) {}
        //! Constructor
        /*!
         * \param maxPoints Capacity
         * \param dim Number of dimensions
         ...
         */
        RBFInterpolant(int maxPoints, int dim, double eta) : RBFInterpolant(maxPoints, dim, arma::zeros(dim), arma::ones(dim), eta) {}
         ...
         * \param eta Damping coefficient (non-negative)
         */
        RBFInterpolant(int maxPoints, int dim, vec xLow, vec xUp, double eta) : RBFInterpolant(maxPoints, dim, xLow, xUp)
        {
            mEta = eta;
        }
        ...
        vec coeffs()
        {
            if (mDirty)
            {
                throw std::logic_error("RBF not updated");
            }
            return mCoeffs;
        }
        ...
        double eval(const vec &ppoint) const
        {
            if (mDirty)
            {
                throw std::logic_error("RBF not updated. You need to call fit() first");
            }
    ...
    public:
         ...
         * \param eta Damping coefficient (non-negative)
         */
        RBFInterpolantCap(int maxPoints, int dim, vec xLow, vec xUp, double eta) : RBFInterpolant<RBFKernel, PolyTail>(maxPoints, dim, xLow, xUp, eta) {}
        //! Constructor with default eta
        /*!
         * \param maxPoints Capacity
         * \param dim Number of dimensions
         ...
         * \param xUp Upper variable bounds
         */
        RBFInterpolantCap(int maxPoints, int dim, vec xLow, vec xUp) : RBFInterpolant<Kernel, Tail>(maxPoints, dim, xLow, xUp) {}
        ...
        void fit()
        {
            ...
            if (this->mDirty)
            {
                ...
                this->mCoeffs = arma::solve(arma::trimatl(
                                                this->mL(arma::span(0, n - 1), arma::span(0, n - 1))),
                                            ff(this->mp(arma::span(0, n - 1))));
                this->mCoeffs = arma::solve(arma::trimatu(
                                                this->mU(arma::span(0, n - 1), arma::span(0, n - 1))),
                                            this->mCoeffs);
                this->mDirty = false;
            }
        }
